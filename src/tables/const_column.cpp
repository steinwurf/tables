...
boost::any const_column::default_value() const { return boost::any(); }

void const_column::add_rows(uint32_t rows) { m_rows += rows; }

void const_column::add_row() { m_rows += 1; }

void const_column::reserve(uint32_t rows) {
  (void)rows; // nothing to do here
}

uint32_t const_column::rows() const { return m_rows; }

uint32_t const_column::empty_rows() const { return 0; }
...
bool const_column::is_constant() const { return true; }
} // namespace tables
